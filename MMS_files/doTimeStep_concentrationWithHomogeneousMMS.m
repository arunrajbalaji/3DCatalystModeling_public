function [dxStripConcentrationOnly, t, residual, A] = doTimeStep_concentrationWithHomogeneousMMS(dxC, dxF, nSpecies, xStripMiddle, xStripOld, xStripOldOld, xStripOldOldOld, isFrontNoFluxCondition, isBackNoFluxCondition,  ...
    t, dt, dtPreviousTimeStep, dtPrevPrevTimeStep, constants, faradaicRxnInfo, isNeutralGas, faradaicRxnSourceTermsStrip, faradaicRxnSourceTermsStrip_init, transportRHSTermsStrip, transportRHSTermsStrip_init, homogeneousRxnSourceTermsStrip, homogeneousRxnSourceTermsStrip_init, stepCtr, debugMode, mmsForcing)
...
for sIndex = 1:nSpecies
    ...
    effectiveDiff = (poro(1:end-1)+poro(2:end))/2.*min(diff(1:end-1,sIndex),diff(2:end,sIndex))./((tort(1:end-1)+tort(2:end))/2);
    ...
    if isFrontNoFluxCondition && ~isNeutralGas(sIndex)
        ...
        mainDiagFront = 2 * effectiveDiff(1)./dxF(1);
%         mainDiagFront = 0;
    end
    ...
    if isBackNoFluxCondition && ~isNeutralGas(sIndex)
        ...
        mainDiagBack = 2 * effectiveDiff(end)./dxF(end);
%         mainDiagBack = 0;
    end

    mainDiag = mainDiag + lhsMult * 1./dxC(2:end-1) .* ([effectiveDiff(2:end-1)./dxF(2:end-1); mainDiagBack] + [mainDiagFront; effectiveDiff(2:end-1)./dxF(2:end-1)]);
    subDiag = subDiag - lhsMult * effectiveDiff(2:end-1)./dxF(2:end-1)./dxC(3:end-1);
    superDiag = superDiag - lhsMult * effectiveDiff(2:end-1)./dxF(2:end-1)./dxC(2:end-2);
    ...
    mainDiag = mainDiag - lhsMult * 1/2 ./ dxC(2:end-1) * (vale(1, sIndex)*e/(kb*T)) .* ([dPhidX(2:end-1) .* effectiveDiff(2:end-1) ; mainDiagBack] - [mainDiagFront; dPhidX(2:end-1).* effectiveDiff(2:end-1) ]);
    subDiag = subDiag + lhsMult * 1/2 ./ dxC(3:end-1) .* effectiveDiff(2:end-1) * (vale(1, sIndex)*e/(kb*T)) .* dPhidX(2:end-1);
    superDiag = superDiag - lhsMult * 1/2 ./ dxC(2:end-2) .* effectiveDiff(2:end-1) * (vale(1, sIndex)*e/(kb*T)) .* dPhidX(2:end-1);
...
for sIndex = 1:nSpecies
    ...
    if stepCtr == 1
        ...
        b(sIndex:nSpecies:end) = b(sIndex:nSpecies:end) + mmsForcing(:, 1, 1, sIndex);
    else
