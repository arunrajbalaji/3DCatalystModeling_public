function [fields3DStarOutput, faradaicRxnSourceTerms, transverseRHSTerms] = doTimeStep_potentialOnly(dxC, dxF, dyC, dyF, dzC, dzF, ...
    nSpecies, fields3DStar, isFrontFluxCondition, isBackFluxCondition, isLeftFluxCondition, isRightFluxCondition, isBottomFluxCondition, isTopFluxCondition, ...
    relaxationParameter, constants, rowInd, colInd, nnz, nEq, ...
    yLeftBCVec, yRightBCVec, zBottomBCVec, zTopBCVec, ...
    ...
    faradaicRxnInfo, isNeutralGas, yRightElectrolyteOverride, bulkElectrolyteL, bulkElectrolyteC, nIter)
...
transverseRHSTerms = zeros(xN-2, yN, zN, nSpecies);
...
for iterIndex = 1:nIter
    ...
    RHSHomogeneousReactions = zeros(xN-2, yN, zN);
    ...
    for sIndex = 1:nSpecies
        ...
        if isFrontFluxCondition && ~isNeutralGas(sIndex)
            xTotalFlux(1, :, :) = 0;
        end
        
        if isBackFluxCondition && ~isNeutralGas(sIndex)
            xTotalFlux(end,: , :) = 0;
        end
        ...
        if isLeftFluxCondition && ~isNeutralGas(sIndex)
             yTotalFlux(:, 1, :) = 0;
        end
        
        if isRightFluxCondition && ~isNeutralGas(sIndex) && ~yRightElectrolyteOverride
            yTotalFlux(:, end, :) = 0;
        end
    ...
    % PRETEND p IS THE DESIRED LINEAR INDEXING INTO THE 3D STRUCTURE
%     zIndex = @(p) (p - mod(p, (xN-2)*yN))/((xN-2)*yN) + 1;
%     yIndex = @(p) ((p - (xN-2)*yN*(zIndex(p)-1)) - mod((p - (xN-2)*yN*(zIndex(p)-1)), xN-2))/(xN-2) + 1;
%     xIndex = @(p) p - (xN-2)*yN*(zIndex(p)-1) - (xN-2)*(yIndex(p)-1);
    ...
    fields3DStar(2*nVar:nVar:end-nVar, 2:end-1, 2:end-1) = fields3DStar(2*nVar:nVar:end-nVar, 2:end-1, 2:end-1) + relaxationParameter * reshape(deltaPhi, [xN-2, yN, zN]);
