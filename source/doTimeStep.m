function [xStripMiddle, t] = doTimeStep(dxC, dxF, dyC, dyFLeft, dyFRight, dzC, dzFBottom, dzFTop, ...
    ...
    faradaicRxnInfo, isNeutralGas, yRightElectrolyteOverride, xStripLeftLeft, dyFLeftLeft, bulkElectrolyteL, bulkElectrolyteC)
        ...
        for xLocIndex = 2:xN-1
            ...
            if yRightElectrolyteOverride
                if (vale(1, speciesIndex) > 0) && (speciesIndex == 3)   % HYDROGEN
                    ...
                    ghostCellOH = -xStripLeft((xLocIndex-1)*(nSpecies+1) + 7) + 2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + 7);
...
%                     ghostCellCharge = sum(vale(1,:)'.* (-xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex-1)*(nSpecies+1)+nSpecies) + 2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex-1)*(nSpecies+1)+nSpecies))) ...
%                         - vale(1,6)'.* (-xStripLeft((xLocIndex-1)*(nSpecies+1)+6) + 2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+6))
%                     xStripRight((xLocIndex-1)*(nSpecies+1) + 6) = -ghostCellCharge;
            ...
            if speciesIndex == 6 && isRightFluxCondition
                2+2;
            end
            ...
            if yRightElectrolyteOverride
                if vale(1, speciesIndex) < 0
                    ...
                    gamma = -alpha - beta;
                    yFluxRightDiff = - effectiveDiff * (alpha * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + speciesIndex) + beta * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + gamma * xStripLeftLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex));
                    yFluxRightElectro = -vale(1,speciesIndex)*effectiveDiff * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + speciesIndex)) ...
                        * (gamma * xStripLeftLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) + beta * xStripLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) + alpha * xStripMiddleTransverse((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)));
                    effectiveBCConc = -1/2 * xStripLeft + 3/2 * xStripMiddleTransverse;
                    yStericSumRight = sum(effectiveBCConc((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1).*stericOnOffVec);
                    yFluxRightSteric = -effectiveDiff * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + speciesIndex)) ...
                        * (log(1 - stericACubed * yStericSumRight) - log(1 - stericACubed * stericSumMiddle))/dyFRight;
                elseif vale(1, speciesIndex) == 0
                    yFluxRightDiff = -(diff(1,1)+diff(1,4))/2 / bulkElectrolyteL * (bulkElectrolyteC(speciesIndex) - (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + speciesIndex)));
                    yFluxRightElectro = 0;
                    yFluxRightSteric = 0;
                end
            end
    ...
    for xLocIndex = 2:xN-1
        ...
        if xLocIndex == 2
            if isFrontFluxCondition
                if isFrontPotentialNeumannCondition
                    dCdXFront = (-xFrontBCValues'./effectiveDiffVec)' - vale(1, :)'*e/kb/T*xFrontPotentialBCValue.*(3/2*xStripMiddle((nSpecies+1)+1:2*(nSpecies+1)-1) - 1/2*xStripMiddle(2*(nSpecies+1)+1:3*(nSpecies+1)-1));
                    dPhidXFront = xFrontPotentialBCValue;
                    diffSumFrontFace = (vale(1, :).*effectiveDiffVec)*dCdXFront;
                    ...
                    dCdXFront = (-xFrontBCValues'./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripMiddle(2*(nSpecies+1)) - xStripMiddle(nSpecies+1))/dxF(1).*(3/2*xStripMiddle((nSpecies+1)+1:2*(nSpecies+1)-1) - 1/2*xStripMiddle(2*(nSpecies+1)+1:3*(nSpecies+1)-1));
                ...
                if isFrontPotentialNeumannCondition
                    diffSumFrontFace = (vale(1, :).*effectiveDiffVec)*(xStripMiddle((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripMiddle((xLocIndex-2)*(nSpecies+1)+1:(xLocIndex-1)*(nSpecies+1)-1))/dxF(xLocIndex-1);
                    dPhidXFront = xFrontPotentialBCValue;
        ...
        if xLocIndex == xN-1
            if isBackFluxCondition
                if isBackPotentialNeumannCondition
                    dCdXBack = (-xBackBCValues'./effectiveDiffVec)' - vale(1, :)'*e/kb/T*xBackPotentialBCValue.*(3/2*xStripMiddle((xN-1)*(nSpecies+1)-nSpecies:(xN-1)*(nSpecies+1)-1) - 1/2*xStripMiddle((xN-2)*(nSpecies+1)-nSpecies:(xN-2)*(nSpecies+1)-1));
                    dPhidXBack = xBackPotentialBCValue;
                    diffSumBackFace = (vale(1, :).*effectiveDiffVec)*dCdXBack;
                    ...
                    dCdXBack = (-xBackBCValues'./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripMiddle(end) - xStripMiddle(end-(nSpecies+1)))/dxF(end).*(3/2*xStripMiddle((xN-1)*(nSpecies+1)-nSpecies:(xN-1)*(nSpecies+1)-1) - 1/2*xStripMiddle((xN-2)*(nSpecies+1)-nSpecies:(xN-2)*(nSpecies+1)-1));
                ...
                if isBackPotentialNeumannCondition
                    diffSumBackFace = (vale(1, :).*effectiveDiffVec)*(xStripMiddle((xLocIndex)*(nSpecies+1)+1:(xLocIndex+1)*(nSpecies+1)-1)-xStripMiddle((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dxF(xLocIndex);
                    dPhidXBack = xBackPotentialBCValue;
        ...
        if isLeftFluxCondition
            if isLeftPotentialNeumannCondition
                dCdYLeft = (-yLeftBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*yLeftPotentialBCValue.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripRight((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumLeftFace = (vale(1, :).*effectiveDiffVec)*dCdYLeft;
                
                dPhidYLeft = yLeftPotentialBCValue;
                electroSumLeft = (vale(1, :).^2.*effectiveDiffVec)*(2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1) - xStripRight((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1));
            else
                dCdYLeft = (-yLeftBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripMiddleTransverse((xLocIndex)*(nSpecies+1)) - xStripLeft((xLocIndex)*(nSpecies+1)))/dyFLeft.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripRight((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumLeftFace = (vale(1, :).*effectiveDiffVec)*dCdYLeft;
            ...
            if isLeftPotentialNeumannCondition
                diffSumLeftFace = (vale(1, :).*effectiveDiffVec)*(xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dyFLeft;
                dPhidYLeft = yLeftPotentialBCValue;
                electroSumLeft = (vale(1, :).^2.*effectiveDiffVec)*xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
            else
                diffSumLeftFace = (vale(1, :).*effectiveDiffVec)*(xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dyFLeft;
                dPhidYLeft = (xStripMiddleTransverse((xLocIndex)*(nSpecies+1)) - xStripLeft((xLocIndex)*(nSpecies+1)))/dyFLeft;
                electroSumLeft = (vale(1, :).^2.*effectiveDiffVec)*xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
        ...
        if isRightFluxCondition
            if isRightPotentialNeumannCondition
                dCdYRight = (-yRightBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*yRightPotentialBCValue.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripLeft((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumRightFace = (vale(1, :).*effectiveDiffVec)*dCdYRight;
                
                dPhidYRight = yRightPotentialBCValue;
                electroSumRight = (vale(1, :).^2.*effectiveDiffVec)*(2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1) - xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1));
            else
                dCdYRight = (-yRightBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripLeft((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumRightFace = (vale(1, :).*effectiveDiffVec)*dCdYRight;
            ...
            if isRightPotentialNeumannCondition
                diffSumRightFace = (vale(1, :).*effectiveDiffVec)*(xStripRight((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dyFRight;
                dPhidYRight = yRightPotentialBCValue;
                electroSumRight = (vale(1, :).^2.*effectiveDiffVec)*xStripRight((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
            else
                diffSumRightFace = (vale(1, :).*effectiveDiffVec)*(xStripRight((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dyFRight;
                dPhidYRight = (xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dyFRight;
                electroSumRight = (vale(1, :).^2.*effectiveDiffVec)*xStripRight((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
        ...
        if yRightElectrolyteOverride
            yFluxRightTotal = zeros(nSpecies, 1);
            for totalFluxIndex = [2, 5, 7]
                yFluxRightDiff = - effectiveDiffVec(totalFluxIndex) * (alpha * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + beta * xStripLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + gamma * xStripLeftLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex));
                yFluxRightElectro = -vale(1,totalFluxIndex)*effectiveDiffVec(totalFluxIndex) * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + 3/2 * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + totalFluxIndex)) ...
                    * (gamma * xStripLeftLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) + beta * xStripLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) + alpha * xStripMiddleTransverse((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)));
                effectiveBCConc = -1/2 * xStripLeft + 3/2 * xStripMiddleTransverse;
                yStericSumRight = sum(effectiveBCConc((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1).*stericOnOffVec);
                yFluxRightSteric = -effectiveDiffVec(totalFluxIndex) * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + 3/2 * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + totalFluxIndex)) ...
                    * (log(1 - stericACubed * yStericSumRight) - log(1 - stericACubed * stericSumMiddle))/dyFRight;
                yFluxRightTotal(totalFluxIndex) = yFluxRightDiff + yFluxRightElectro + yFluxRightSteric;
            end
            
            for totalFluxIndex = [1, 4]
                yFluxRightTotal(totalFluxIndex) = -(diff(1,1)+diff(1,4))/2 / bulkElectrolyteL * (bulkElectrolyteC(speciesIndex) - (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1) + speciesIndex)));
            end
            
            dCdYRight = zeros(nSpecies, 1);
            dCdYRight([2,5,7]) = (-yFluxRightTotal([2,5,7])./effectiveDiffVec([2,5,7])') - vale(1, [2,5,7])'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddleTransverse([(xLocIndex-1)*(nSpecies+1)+2, (xLocIndex-1)*(nSpecies+1)+5, (xLocIndex-1)*(nSpecies+1)+7]) - 1/2*xStripLeft([(xLocIndex-1)*(nSpecies+1)+2, (xLocIndex-1)*(nSpecies+1)+5, (xLocIndex-1)*(nSpecies+1)+7]));
            dCdYRight([1,4]) = (-yFluxRightTotal([1,4])./effectiveDiffVec([1,4])') - vale(1, [1,4])'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddleTransverse([(xLocIndex-1)*(nSpecies+1)+1, (xLocIndex-1)*(nSpecies+1)+4]) - 1/2*xStripLeft([(xLocIndex-1)*(nSpecies+1)+1, (xLocIndex-1)*(nSpecies+1)+4]));
            dCdYRight([3,6]) = (xStripRight([(xLocIndex-1)*(nSpecies+1)+3, (xLocIndex-1)*(nSpecies+1)+6])-xStripMiddleTransverse([(xLocIndex-1)*(nSpecies+1)+3, (xLocIndex-1)*(nSpecies+1)+6]))/dyFRight;
%             dCdYRight(3) = (-yRightBCValues(3)./effectiveDiffVec(3))' - vale(1, 3)'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+3) - 1/2*xStripLeft((xLocIndex-1)*(nSpecies+1)+3));
%             dCdYRight(6) = (-0./effectiveDiffVec(6))' - vale(1, 6)'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+6) - 1/2*xStripLeft((xLocIndex-1)*(nSpecies+1)+6));
            diffSumRightFace = (vale(1, :).*effectiveDiffVec)*dCdYRight;
        ...
        if isBottomFluxCondition
            if isBottomPotentialNeumannCondition
                dCdZBottom = (-zBottomBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*zBottomPotentialBCValue.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripTop((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumBottomFace = (vale(1, :).*effectiveDiffVec)*dCdZBottom;
                
                dPhidZBottom = zBottomPotentialBCValue;
                electroSumBottom = (vale(1, :).^2.*effectiveDiffVec)*(2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1) - xStripTop((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1));
            else
                dCdZBottom = (-zBottomBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripMiddleTransverse((xLocIndex)*(nSpecies+1)) - xStripBottom((xLocIndex)*(nSpecies+1)))/dzFBottom.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripTop((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumBottomFace = (vale(1, :).*effectiveDiffVec)*dCdZBottom;
            ...
            if isBottomPotentialNeumannCondition
                diffSumBottomFace = (vale(1, :).*effectiveDiffVec)*(xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripBottom((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dzFBottom;
                dPhidZBottom = zBottomPotentialBCValue;
                electroSumBottom = (vale(1, :).^2.*effectiveDiffVec)*xStripBottom((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
            else
                diffSumBottomFace = (vale(1, :).*effectiveDiffVec)*(xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripBottom((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dzFBottom;
                dPhidZBottom = (xStripMiddleTransverse((xLocIndex)*(nSpecies+1)) - xStripBottom((xLocIndex)*(nSpecies+1)))/dzFBottom;
                electroSumBottom = (vale(1, :).^2.*effectiveDiffVec)*xStripBottom((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
        ...
        if isTopFluxCondition
            if isTopPotentialNeumannCondition
                dCdZTop = (-zTopBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*zTopPotentialBCValue.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripBottom((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumTopFace = (vale(1, :).*effectiveDiffVec)*dCdZTop;
                
                dPhidZTop = zTopPotentialBCValue;
                electroSumTop = (vale(1, :).^2.*effectiveDiffVec)*(2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1) - xStripBottom((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1));
            else
                dCdZTop = (-zTopBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripTop((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dzFTop.*(3/2*xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripBottom((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumTopFace = (vale(1, :).*effectiveDiffVec)*dCdZTop;
            ...
            if isTopPotentialNeumannCondition
                diffSumTopFace = (vale(1, :).*effectiveDiffVec)*(xStripTop((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dzFTop;
                dPhidZTop = zTopPotentialBCValue;
                electroSumTop = (vale(1, :).^2.*effectiveDiffVec)*xStripTop((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
            else
                diffSumTopFace = (vale(1, :).*effectiveDiffVec)*(xStripTop((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1)-xStripMiddleTransverse((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1))/dzFTop;
                dPhidZTop = (xStripTop((xLocIndex)*(nSpecies+1)) - xStripMiddleTransverse((xLocIndex)*(nSpecies+1)))/dzFTop;
                electroSumTop = (vale(1, :).^2.*effectiveDiffVec)*xStripTop((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1);
