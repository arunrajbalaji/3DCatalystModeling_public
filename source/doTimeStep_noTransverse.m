function [xStripMiddle, t] = doTimeStep_noTransverse(dxC, dxF, dyC, dyFLeft, dyFRight, dzC, dzFBottom, dzFTop, ...
...
%                     xStripRight((xLocIndex-1)*(nSpecies+1) + 6) = -ghostCellCharge;
            ...
            yFluxRightElectro = -vale(1,speciesIndex)*effectiveDiff * (xStripRight((xLocIndex-1)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex))/2 ...
            ...
            if yRightElectrolyteOverride
                if vale(1, speciesIndex) < 0
                    ...
                    yFluxRightElectro = -vale(1,speciesIndex)*effectiveDiff * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex)) ...
                ...
                elseif vale(1, speciesIndex) == 0
                    yFluxRightDiff = -(diff(1,1)+diff(1,4))/2 / bulkElectrolyteL * (bulkElectrolyteC(speciesIndex) - (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex)));
                    yFluxRightElectro = 0;
                    yFluxRightSteric = 0;
                end
            end
    ...
    for xLocIndex = 2:xN-1
        ...
        if isRightFluxCondition
            if isRightPotentialNeumannCondition
                ...
                dCdYRight = (-yRightBCValues./effectiveDiffVec)' - vale(1, :)'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddle((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddle((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1) - 1/2*xStripLeft((xLocIndex-1)*(nSpecies+1)+1:xLocIndex*(nSpecies+1)-1));
                diffSumRightFace = (vale(1, :).*effectiveDiffVec)*dCdYRight;
                
                dPhidYRight = (xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddle((xLocIndex)*(nSpecies+1)))/dyFRight;
                electroSumRight = (vale(1, :).^2.*effectiveDiffVec)*(2*xStripMiddle((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1) - xStripLeft((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1));
            end
        else
            if isRightPotentialNeumannCondition
        ...
        if yRightElectrolyteOverride
            ...
            for totalFluxIndex = [2, 5, 7]
                yFluxRightDiff = - effectiveDiffVec(totalFluxIndex) * (alpha * xStripMiddle((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + beta * xStripLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + gamma * xStripLeftLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex));
                yFluxRightElectro = -vale(1,totalFluxIndex)*effectiveDiffVec(totalFluxIndex) * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + 3/2 * xStripMiddle((xLocIndex-1)*(nSpecies+1) + totalFluxIndex)) ...
                    * (gamma * xStripLeftLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) + beta * xStripLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) + alpha * xStripMiddle((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)));
                effectiveBCConc = -1/2 * xStripLeft + 3/2 * xStripMiddle;
                yStericSumRight = sum(effectiveBCConc((xLocIndex-1)*(nSpecies+1)+1:(xLocIndex)*(nSpecies+1)-1).*stericOnOffVec);
                yFluxRightSteric = -effectiveDiffVec(totalFluxIndex) * (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + totalFluxIndex) + 3/2 * xStripMiddle((xLocIndex-1)*(nSpecies+1) + totalFluxIndex)) ...
                    * (log(1 - stericACubed * yStericSumRight) - log(1 - stericACubed * stericSumMiddle))/dyFRight;
                yFluxRightTotal(totalFluxIndex) = yFluxRightDiff + yFluxRightElectro + yFluxRightSteric;
            end
            
            for totalFluxIndex = [1, 4]
                yFluxRightTotal(totalFluxIndex) = -(diff(1,1)+diff(1,4))/2 / bulkElectrolyteL * (bulkElectrolyteC(speciesIndex) - (-1/2 * xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + 3/2 * xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex)));
            end
            
            dCdYRight = zeros(nSpecies, 1);
            dCdYRight([2,5,7]) = (-yFluxRightTotal([2,5,7])./effectiveDiffVec([2,5,7])') - vale(1, [2,5,7])'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddle((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddle([(xLocIndex-1)*(nSpecies+1)+2, (xLocIndex-1)*(nSpecies+1)+5, (xLocIndex-1)*(nSpecies+1)+7]) - 1/2*xStripLeft([(xLocIndex-1)*(nSpecies+1)+2, (xLocIndex-1)*(nSpecies+1)+5, (xLocIndex-1)*(nSpecies+1)+7]));
            dCdYRight([1,4]) = (-yFluxRightTotal([1,4])./effectiveDiffVec([1,4])') - vale(1, [1,4])'*e/kb/T*(xStripRight((xLocIndex)*(nSpecies+1)) - xStripMiddle((xLocIndex)*(nSpecies+1)))/dyFRight.*(3/2*xStripMiddle([(xLocIndex-1)*(nSpecies+1)+1, (xLocIndex-1)*(nSpecies+1)+4]) - 1/2*xStripLeft([(xLocIndex-1)*(nSpecies+1)+1, (xLocIndex-1)*(nSpecies+1)+4]));
            dCdYRight([3,6]) = (xStripRight([(xLocIndex-1)*(nSpecies+1)+3, (xLocIndex-1)*(nSpecies+1)+6])-xStripMiddle([(xLocIndex-1)*(nSpecies+1)+3, (xLocIndex-1)*(nSpecies+1)+6]))/dyFRight;
