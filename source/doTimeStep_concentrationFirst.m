function [xStripMiddle, t] = doTimeStep_concentrationFirst(dxC, dxF, dyC, dyFLeft, dyFRight, dzC, dzFBottom, dzFTop, ...
...
rightRHSDiffTermStored = zeros(xN-2, nSpecies);
leftRHSDiffTermStored = zeros(xN-2, nSpecies);
rightRHSStericTermStored = zeros(xN-2, nSpecies);
leftRHSStericTermStored = zeros(xN-2, nSpecies);
...
%                     xStripRight((xLocIndex-1)*(nSpecies+1) + 6) = -ghostCellCharge;
            ...
            rightRHSDiffTermStored(xLocIndex-1, speciesIndex) = rightRHSDiffTerm;
            leftRHSDiffTermStored(xLocIndex-1, speciesIndex) = leftRHSDiffTerm;
            rightRHSStericTermStored(xLocIndex-1, speciesIndex) = rightRHSStericTerm;
            leftRHSStericTermStored(xLocIndex-1, speciesIndex) = leftRHSStericTerm;
            ...
            if xLocIndex == 2 && isFrontFluxCondition && ~isNeutralGas(speciesIndex)
                b(rowInd(ctr-1)) = -poro(xLocIndex)*(xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex)-xStripOld((xLocIndex-1)*(nSpecies+1) + speciesIndex))/dt...
...
%     warning('off', 'MATLAB:nearlySingularMatrix')
    ...
    for xLocIndex = 2:xN-1
        % Recompute right hand side fluxes (currents) based on updated phi
        % values (electromigration in particular)
        
        for speciesIndex = 1:nSpecies
            % Override required at y Right boundary
            yFluxRightElectro = -vale(1,speciesIndex)*e/(kb*T)*effectiveDiff * (xStripRight((xLocIndex-1)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex))/2 ...
                * (xStripRight((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) - xStripMiddle((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1))) / dyFRight;
            if yRightElectrolyteOverride
                if vale(1, speciesIndex) < 0
            ...
            % No override required at other boundaries
            yFluxLeftElectro = -vale(1,speciesIndex)*e/(kb*T)*effectiveDiff * (xStripLeft((xLocIndex-1)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex))/2 ...
                * (xStripMiddle((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1)) - xStripLeft((xLocIndex-1 - 1)*(nSpecies+1) + (nSpecies+1) + (nSpecies+1))) / dyFLeft;
            if isLeftFluxCondition && ~isNeutralGas(speciesIndex)
                yFluxLeftElectro = 0;
            ...
            if xLocIndex == 2 && isFrontFluxCondition && ~isNeutralGas(speciesIndex)
                xRHSTotal(xLocIndex-1, speciesIndex) = outerDerivativeBackFace*(rightElectromigrationTerm*(xStripMiddle((xLocIndex)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex)) + rightRHSDiffTermStored(xLocIndex-1, speciesIndex)  + rightRHSStericTermStored(xLocIndex-1, speciesIndex)) ...
                    + 1/dxC(2) * xFrontBCValues(speciesIndex);
                
            elseif xLocIndex == xN-1 && isBackFluxCondition && ~isNeutralGas(speciesIndex)
                xRHSTotal(xLocIndex-1, speciesIndex) = - outerDerivativeFrontFace*(leftElectromigrationTerm*(xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-2)*(nSpecies+1) + speciesIndex)) + leftRHSDiffTermStored(xLocIndex-1, speciesIndex) + leftRHSStericTermStored(xLocIndex-1, speciesIndex)) ...
                    - 1/dxC(end-1) * xBackBCValues(speciesIndex);
            else
                xRHSTotal(xLocIndex-1, speciesIndex) = outerDerivativeBackFace*(rightElectromigrationTerm*(xStripMiddle((xLocIndex)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex)) + rightRHSDiffTermStored(xLocIndex-1, speciesIndex) + rightRHSStericTermStored(xLocIndex-1, speciesIndex)) ...
                    - outerDerivativeFrontFace*(leftElectromigrationTerm*(xStripMiddle((xLocIndex-1)*(nSpecies+1) + speciesIndex) + xStripMiddle((xLocIndex-2)*(nSpecies+1) + speciesIndex)) + leftRHSDiffTermStored(xLocIndex-1, speciesIndex) + leftRHSStericTermStored(xLocIndex-1, speciesIndex));
            end
        end
        
        % Convenient summations for LHS matrix entries
...
% Add reactions here once confident in current conservation solve
    ...
    [maxValue, maxIndex] = max(abs(dxStripPhiOnly));
    disp(['Max dV: ' num2str(sign(dxStripPhiOnly(maxIndex)) * maxValue) ' V']);
